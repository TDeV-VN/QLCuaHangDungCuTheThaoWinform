    ...
    public partial class FormSuaChiTietSP : Form
    {
        ...
        public void populateImages()
        {
            ...
            if(urlAvatar != "")
            {
                ...
                avatar.FileName = Path.GetFileName(urlAvatar);
            ...
            for (int i = 0; i < lstImg.Length; i++)
            {
                ...
                lstImg[i].FileName = Path.GetFileName(urlNonAvatar[i]);
        ...
        private void ListImageButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            int imageIndex = flowLayoutPanel1.Controls.IndexOf(clickedButton.Parent); // Assuming the button is directly inside the image control
            ...
            if (imageIndex >= 0 && imageIndex < urlNonAvatar.Count)
            {
                ...
                {
                    flowLayoutPanel1.Controls.RemoveAt(imageIndex); // Update your UI to reflect the removal (e.g., remove the image control)
                    suaChiTietSPBLL.deleteImage(product.MaSP, urlNonAvatar[imageIndex]);
                    urlNonAvatar.RemoveAt(imageIndex); // Remove the URL from the list
                }
        ...
        private void AvatarButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                panelAvatar.Controls.RemoveAt(0); // Update your UI to reflect the removal (e.g., remove the image control)
                suaChiTietSPBLL.deleteImage(product.MaSP, urlAvatar);
                urlAvatar = ""; // Remove the URL from the list
            }
        }
        ...
        private void GrVDanhMuc_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0) // Check if a valid row index is clicked
            {
                ...
                // Now you can access data from the clicked row
        ...
        {
            // Allow only digits (0-9) and control keys (e.g., Backspace, Delete)
